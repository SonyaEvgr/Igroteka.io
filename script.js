...
window.doLogin = function() {
    ...
    if (!username || !password) { showToast('Заполните все поля', 'warning'); return; }
    const users = getUsers();
...
window.doRegister = function() {
    ...
    if (!name || !username || !password || !confirm) { showToast('Заполните все поля', 'warning'); return; }
    ...
    const users = getUsers();
    if (users.find(u => u.username === username)) { showToast('Логин занят', 'error'); return; }
    const newUser = { username, password, name, role: users.length === 0 ? 'admin' : 'user', xp: 0, joinedAt: new Date().toISOString() };
    users.push(newUser);
    saveUsers(users);
...
function loadChatMessages() {
    ...
    const myChat = chats.find(c => c.userId === currentUser.username) || { messages: [] };
    const container = document.getElementById('chatMessages');
    if (!container) return;

    if (myChat.messages.length === 0) {
        container.innerHTML = `<div style="text-align:center;color:#aaa;padding:30px;font-size:0.9rem"><i class="fas fa-comments" style="font-size:2rem;display:block;margin-bottom:10px"></i>Напишите нам — мы поможем!</div>`;
        return;
    }
    container.innerHTML = myChat.messages.map(m => {
        const mine = m.from === currentUser.username;
        return `<div class="chat-msg ${mine ? 'mine' : 'theirs'}">
            <div class="msg-bubble">${escHtml(m.text)}</div>
            <div class="msg-meta">${mine ? 'Вы' : m.fromName || 'Поддержка'} · ${formatDate(m.at)}</div>
        </div>`;
    }).join('');
...
window.sendChatMessage = function() {
    ...
    if (!text) return;
    if (!currentUser) { showToast('Войдите для отправки сообщений', 'warning'); return; }

    const chats = getChats();
    let ticket = chats.find(c => c.userId === currentUser.username);
    if (!ticket) {
        ticket = { id: Date.now(), userId: currentUser.username, userName: currentUser.name || currentUser.username, status: 'open', createdAt: new Date().toISOString(), messages: [] };
        chats.push(ticket);
    }
    ticket.messages.push({ from: currentUser.username, fromName: currentUser.name || currentUser.username, text, at: new Date().toISOString() });
    ticket.status = 'open';
    saveChats(chats);
    inp.value = '';
    loadChatMessages();
...
function renderTicket(ticket, showReply) {
    ...
    const statusBadge = `<span class="ticket-status ticket-${ticket.status||'open'}">${ticket.status === 'closed' ? 'Закрыто' : 'Открыто'}</span>`;
    const replyHtml = showReply ? `
        <div class="support-reply-area">
            <input id="reply_${ticket.id}" placeholder="Ответить пользователю..." onkeydown="if(event.key==='Enter')replyToTicket('${ticket.id}')">
            <button class="btn-primary btn-sm" onclick="replyToTicket('${ticket.id}')"><i class="fas fa-paper-plane"></i></button>
            <button class="btn-secondary btn-sm" onclick="closeTicket('${ticket.id}')">Закрыть</button>
        </div>` : '';
    ...
    return `<div class="support-ticket">
        <div class="support-ticket-header" onclick="this.nextElementSibling.style.display=this.nextElementSibling.style.display==='none'?'flex':'none'">
            <span><strong>${escHtml(ticket.userName||ticket.userId)}</strong> · #${ticket.id}</span>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Мобильное сворачивание категорий
    if (window.innerWidth <= 768) {
        document.getElementById('categoriesList')?.classList.add('collapsed');
    }
});
